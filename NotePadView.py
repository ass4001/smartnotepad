...
class Notepad:
    ...
    def set_tool_bar(self):
        ...
        self.mike_btn = tk.Button(self.tool_bar, image=self.mike_icon)
        self.mike_btn.grid(row=0, column=9, padx=5)

        ##Date
        today = datetime.now()
        self.today_date = tk.Label(self.tool_bar,text=f'{ today.strftime("%d-%b-%Y") }',fg="Black",font=("Arial",15,"bold"))
        self.today_date.grid(row=0,column=10,sticky=tk.W,padx=50)

       ##Running time 
        self.running_time = tk.StringVar()
        self.run_time = tk.Label(self.tool_bar,textvariable=self.running_time,fg="Black",font=("Arial",15,"bold"))
        self.run_time.grid(row=0,column=11,sticky=tk.W,padx=50)
        self.t1 = Thread(target=self.set_run_time)
        self.t1.daemon = True
        self.t1.start()

    def set_run_time(self):
        try:
            if int(time.time()-self.start_run) < 60:
                self.running_time.set(f"00:00:{int(time.time()-self.start_run)}")
            elif 60 <= int(time.time()-self.start_run) < 60*60:
                self.running_time.set(
                    f"00:{(int(time.time()-self.start_run)//60)}:{(int(time.time()-self.start_run)%60)}")
            elif int(time.time()-self.start_run) >= 60*60:
                self.running_time.set(
                    f"{(int(time.time()-self.start_run)//60*60)}:{(int(time.time()-self.start_run)%60*60)}:{(int(time.time()-self.start_run)%(60*60*60))}")
            time.sleep(0.10)
            self.set_run_time()
        except:
    ...
    def set_tool_bar_event_bindings(self):
        ...
        self.M_thread = Thread(target=self.say_something)
        self.M_thread.daemon =True
        self.mike_btn.configure(command=lambda e=0:self.M_thread.start())
    ...
    def set_file_menu_event_bindings(self):
        ...
        self.root.bind("<Control-a>", lambda e: self.M_thread.start())
        self.root.bind("<Control-A>", lambda e: self.M_thread.start())
    ...
    def destroy_find_dialog(self , e=0):
        # self.text_editor.tag_config(
            # 'match', foreground=self.text_editor["fg"], background=self.text_editor["bg"])
        self.text_editor.tag_delete('match')
    ...
    def find_func(self, event=None):
        ...
        self.find_dialogue.protocol("WM_DELETE_WINDOW",self.destroy_find_dialog)
    ...
    def say_something(self):
        self.mike_btn.configure(state="disable",bg="lightblue")
        print("chala")
        self.M_thread = Thread(target=self.say_something)
        try:
            self.takeAudio = self.file_controller.take_quary()
            ...
            self.mike_btn.configure(state="active",bg="White")
    ...
    def change_font_color(self):
        ...
        self.text_editor.configure(fg=c[1])
    ...
    def change_bold(self):
        try:
            if self.font_bold=="bold":
                self.font_bold="normal"
            else:
                self.font_bold="bold"
            self.text_editor.config(
                font=(self.current_font_family,self.current_font_size,self.font_bold))
        except:
            self.text_editor.config(
                font=(f"{self.current_font_family}", f" {self.current_font_size}","bold"))
            self.font_bold="bold"


    def change_italic(self):
        try:
            if self.font_italic=="italic":
                self.font_italic ="normal"
            else:
                self.font_italic="italic"
            self.text_editor.config(
                font=(f"{self.current_font_family}", f" {self.current_font_size}",self.font_italic))
        except:
            self.text_editor.config(
                font=(f"{self.current_font_family}", f" {self.current_font_size}",'italic'))
            self.font_italic="italic"
        finally:
            print(self.text_editor["font"])
        
    def change_underline(self):
        try:
            und = self.font_edit.actual("underline")
            und = not und
            self.font_edit.config(underline=und)
            self.text_editor.config(font=self.font_edit)
        except:
            self.font_edit =font.Font(underline=True)
            self.text_editor.config(font=self.font_edit)
    ...
    def get_file_pwd(self):
        ...
        "Password", "Please Enter Acceess Password")
        return access_pwd
